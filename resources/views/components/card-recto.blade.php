...
<div class="card bg{{$current_family}} text-white w-[5.71cm] h-[8.89cm] m-auto"
    ...
    <div class="card-body justify-between" :style="{ 'padding': padding+'px' }">
        ...
        <label for="photo_families.{{$current_family}}.family_members.{{$family_member}}" class="avatar justify-center cursor-pointer" x-show="display_photo">
            <div
                class="group photo-upload w-20 rounded-full ring ring-white ring-offset-white ring-offset-2"
            ...
            >
                ...
                <!-- Overlay -->
                <div @click="loading = true" class="hidden group-hover:flex items-center justify-center absolute bg-black bg-opacity-50 rounded-full w-full h-full top-0 text-white text-xs text-center font-normal font-standard">
